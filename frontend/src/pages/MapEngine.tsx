          ...
          <div className="w-96 flex flex-col gap-4">
            <div className="bg-gray-900/80 border border-gray-800 rounded-xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['x', 'y', 'rx', 'ry'] as const).map(field => (
                  <div key={field}>
                    <label className="text-[10px] uppercase font-bold text-gray-500 block mb-1">
                      {t(`map_engine.spawn_${field}` as any)}
                    </label>
                    <input
                      type="number"
                      value={spawnForm[field]}
                      onChange={e => setSpawnForm(f => ({ ...f, [field]: parseInt(e.target.value) || 0 }))}
                      className="w-full bg-gray-950 border border-gray-700 rounded-lg px-3 py-2 text-sm text-center text-cyan-300 focus:outline-none focus:border-teal-500"
                    />
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                >
                  {spawnForm.monsterName
                    ...
                    : t('map_engine.spawn_select_monster' as any)}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] uppercase font-bold text-gray-500 block mb-1">{t('map_engine.spawn_amount' as any)}</label>
                  <input
                    type="number"
                    min={1}
                    value={spawnForm.amount}
                    onChange={e => setSpawnForm(f => ({ ...f, amount: parseInt(e.target.value) || 1 }))}
                    className="w-full bg-gray-950 border border-gray-700 rounded-lg px-3 py-2 text-sm text-center text-emerald-400 focus:outline-none focus:border-teal-500"
                  />
                </div>
                <div>
                  <label className="text-[10px] uppercase font-bold text-gray-500 block mb-1">{t('map_engine.spawn_respawn' as any)}</label>
                  <input
                    type="number"
                    min={0}
                    step={1000}
                    value={spawnForm.respawn}
                    onChange={e => setSpawnForm(f => ({ ...f, respawn: parseInt(e.target.value) || 0 }))}
                    className="w-full bg-gray-950 border border-gray-700 rounded-lg px-3 py-2 text-sm text-center text-yellow-400 focus:outline-none focus:border-teal-500"
                  />
                </div>
              </div>
